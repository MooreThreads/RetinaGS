# modified from train_ddp
# sincerely, it's not a good idea to train gs with ddp, and multiple-machine ddp could be even worse 
import os
import torch
...
def ddp_setup(rank, world_size, LOCAL_RANK, MASTER_ADDR, MASTER_PORT):
    """
    Args:
...
def main(rank: int, world_size: int, LOCAL_RANK: int, MASTER_ADDR, MASTER_PORT, train_args):
    ddp_setup(rank, world_size, LOCAL_RANK, MASTER_ADDR, MASTER_PORT)
    training(*train_args)
...
if __name__ == "__main__":
    ...
    if RANK == 0 and (not os.path.exists(ply_path)):
