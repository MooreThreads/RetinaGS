...
def readColmapEmptyCameras(cam_extrinsics, cam_intrinsics, images_folder):
    cam_infos = []
    for idx, key in enumerate(cam_extrinsics):
        sys.stdout.write('\r')
        # the exact output you're looking for:
        ...
        image_path = os.path.join(images_folder, os.path.basename(extr.name))
        image_name = os.path.basename(image_path).split(".")[0]
        image = None # image = Image.open(image_path)
...
def readEmptyCamerasFromTransforms(path, transformsfile, white_background, extension=".png"):
    cam_infos = []
...
def readNerfSyntheticAndEmptyCameraInfo(path, white_background, eval, pointcloud_sample_rate=1, points3D="points3d", extension=""):
    ...
    print("Reading Test Transforms")
    test_cam_infos = readEmptyCamerasFromTransforms(path, "transforms_test.json", white_background, extension)
