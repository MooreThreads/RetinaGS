...
def render_set(model_path, name, iteration, scene:SimpleScene, gaussians, pipeline, background, skip_lpips, save_step, save_activated_ply, full_dict, per_view_dict, target_scale):
    ...
    if name == 'train':
        ...
        cameras_r4:CameraListDataset = scene.getTrainCameras(scale=target_scale)
    else:    
        cameras_r1:CameraListDataset = scene.getTestCameras(scale=1)
        cameras_r4:CameraListDataset = scene.getTestCameras(scale=target_scale)
    ...
    for idx, datar1_r4 in enumerate(tqdm(zip(train_loader_r1, train_loader_r4), desc="metric progress")):        
        ...
        """
        rendering = torch.nn.functional.interpolate(rendering_r4.unsqueeze(0), gt.shape[-2:] , mode='nearest').squeeze(0)
...
def render_sets(dataset : ModelParams, opt, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, skip_lpips : bool, save_step : int, save_activated_ply: bool, target_scale: float):
    gaussians = GaussianModel(dataset.sh_degree)
    scene = SimpleScene(dataset, load_iteration=iteration, resolution_scales=[1, target_scale]) # scene = Scene(dataset, gaussians, load_iteration=iteration, shuffle=False)
    ...
    if not skip_test:
        print('test')
        render_set(dataset.model_path, "test", scene.loaded_iter, scene, gaussians, pipeline, background, skip_lpips, save_step, save_activated_ply, full_dict, per_view_dict, target_scale)
    
    if not skip_train:
        print("train")
        render_set(dataset.model_path, "train", scene.loaded_iter, scene, gaussians, pipeline, background, skip_lpips, save_step, save_activated_ply, full_dict, per_view_dict, target_scale)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--original_resoultion", default=-1, type=float)  
    parser.add_argument("--target_resoultion", default=-1, type=float)    
    ...
    target_scale = args.original_resoultion/args.target_resoultion
    render_sets(model.extract(args), op.extract(args), args.iteration, pipeline.extract(args), args.skip_train, args.skip_test, args.skip_lpips, args.save_step, args.save_activated_ply, target_scale)