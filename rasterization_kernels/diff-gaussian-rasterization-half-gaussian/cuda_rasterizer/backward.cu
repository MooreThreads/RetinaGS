...
renderCUDA(
	...
	const float* __restrict__ range_low,
	const float* __restrict__ range_up,
	const float2* __restrict__ points_zw_image,
...
)
{
	...
	__shared__ float2 collected_zw[BLOCK_SIZE];
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			collected_zw[block.thread_rank()] = points_zw_image[coll_id];
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			float2 zw = collected_zw[j];
			// homogeneous coordinate of the ray and 2D Gaussian
			float3 p_hom = {
				pix2Ndc(pixf.x, W) * (zw.y + 0.0000001f) * tan_fovx,
				pix2Ndc(pixf.y, H) * (zw.y + 0.0000001f) * tan_fovy,
				zw.y
			};
			float4 p_world = transformPoint4x4(p_hom, projmatrix_inv);
			if (
				(p_world.x <= range_low[0]) || (p_world.x > range_up[0]) || 
				(p_world.y <= range_low[1]) || (p_world.y > range_up[1]) || 
				(p_world.z <= range_low[2]) || (p_world.z > range_up[2])
			) continue;	
...
void BACKWARD::render(
	...
	const float* range_low, const float* range_up,
	const float2* points_zw_image,
