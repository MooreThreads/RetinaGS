...
def training(args, dataset_args, opt, pipe, testing_iterations, ply_iteration, checkpoint_iterations, debug_from, LOGGERS):
    ...
    current_time = time.strftime("%Y_%m_%d_%H_%M_%S", time.localtime()) 
    tmp_str = 'afetr build model memory_allocted {}'.format(torch.cuda.memory_allocated()/(1024**2))
    logger.info(tmp_str)
    pick.append(tmp_str + '\n')
    ...
    tmp_str = 'after load data memory_allocted: {}'.format(torch.cuda.memory_allocated()/(1024**2))
    logger.info(tmp_str)
    pick.append(tmp_str + '\n')
    ...
    tmp_str = 'after training peak_memory_allocted: {}'.format(torch.cuda.max_memory_allocated()/(1024**2))
    logger.info(tmp_str)
    pick.append(tmp_str + '\n')
    ...
    pick += [lines[i]+'\n' for i in range(3)]
    pick += [line+'\n' for line in lines if 'ncclKernel' in line]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--DIVIDE_SPACE", action='store_true', default=False)
